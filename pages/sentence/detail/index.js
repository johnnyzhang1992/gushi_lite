...
Page({
    ...
    getSentenceDetail: function(sentence_id,user_id){
        ...
        return new Promise((resolve,reject)=> { //结果以Promise形式返回
            http.request(app.globalData.url+'/wxxcx/getSentenceDetail/'+sentence_id,{user_id: user_id}).then(res=>{
    ...
    drawImage: function(file_path){
        ...
        canvas.drawText(ctx,that.data.sentence.title,winWidth * pixelRatio/2,(winHeight-110-text_y)*pixelRatio,'center',
    ...
    onLoad: function (options) {
        ...
        that.getSentenceDetail(options.id,that.data.user_id).then((res)=>{
            if(res && res.succeeded){
                wx.hideLoading();
    ...
    updateCollect: function () {
        ...
        if(that.data.user_id<1){
            ...
            authLogin.authLogin('/pages/poem/detail/index?id='+that.data.sentence.id,'nor',app);
        }else{
            let data = {
                user_id: that.data.user_id,
                wx_token: wx.getStorageSync('wx_token')
            };
            http.request(app.globalData.url+'/wxxcx/'+that.data.sentence.id + '/collect/sentence',data).then(res=>{
               if(res.data && res.succeeded){
                   that.setData({
    ...
    onPullDownRefresh: function () {
        ...
        that.getSentenceDetail(that.data.sentence.id,that.data.user_id).then((res)=>{
            if(res && res.succeeded){
                wx.hideLoading();
